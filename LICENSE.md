...
- **원문 시작 부분(Preface):**
  ...
  <details>
    <summary><em>번역</em></summary>

    > 이 문서는 타입 세이프하고 성능이 우수하며 종속성이 없는 bean 매핑 코드를 생성하기 위한 애노테이션 프로세서인 MapStruct의 참조 문서입니다. 이 가이드는 MapStruct에서 제공하는 모든 기능을 다룹니다. 이 가이드에서 모든 질문에 대한 답이 나오지 않는다면 [MapStruct GitHub](https://github.com/mapstruct/mapstruct/discussions) 토론에 참여하여 도움을 받으세요.
    > 
    > 이 가이드에서 오타나 기타 오류를 발견하셨나요? [MapStruct GitHub 레포지토리](https://github.com/mapstruct/mapstruct)에서 이슈를 개설하여 알려 주시거나, 더 좋은 방법은 커뮤니티에 도움을 주시고 풀 리퀘스트를 보내서 수정해 주세요!
    > 
    > 이 저작물은 [Creative Commons Attribution-ShareAlike 4.0 국제 라이선스](http://creativecommons.org/licenses/by-sa/4.0/)에 따라 라이센스가 부여되어 있습니다.
...
- **原文（Preface）:**
  ...
  <details>
    <summary><em>翻訳</em></summary>

    > この文書は、型安全で性能が優れており、依存関係のない bean マッピングコードを生成するためのアノテーションプロセッサである MapStruct のリファレンスドキュメントです。  
    > このガイドでは、MapStruct が提供するすべての機能を扱います。もしこのガイドで全ての質問に対する答えが見つからない場合は、[MapStruct GitHub](https://github.com/mapstruct/mapstruct/discussions) のディスカッションに参加して、助けを得てください。
    > 
    > このガイドで誤字やその他のエラーを発見された場合は、[MapStruct GitHub リポジトリ](https://github.com/mapstruct/mapstruct)でイシューを作成してお知らせいただくか、あるいはコミュニティの助けとなるためにプルリクエストを送って修正してください!
    > 
    > 本著作物は、[Creative Commons Attribution-ShareAlike 4.0 国際ライセンス](http://creativecommons.org/licenses/by-sa/4.0/)に基づいてライセンスが付与されています.
